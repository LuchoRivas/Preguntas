    ...
    public class AccountController : Controller
    {
                ...
                default:
                    if (model.isMobile == true)
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(model.Password))
                            ...
                            error.Add(new { Input = "Usuario", Msg = "Usuario/Contraseña invalido" });
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                UserManager.AddPassword(usuario.UsuarioApp.Id, model.Password);
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> RegisterMobile(RegisterMobileViewModel model)
        {
            var error = new List<object>();
            if (ModelState.IsValid && model.esMobile == true)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                var db = new ApplicationDbContext();
                model.Token = Guid.NewGuid().ToString();
                ...
                return Json(new { Result = usuario, Token = model.Token, Error = "" }, JsonRequestBehavior.AllowGet);
            }
            ...
            if (string.IsNullOrWhiteSpace(model.NickName))
                error.Add(new { Input = "Nickname", Msg = "El campo nickname esta vacio" });
            if (string.IsNullOrWhiteSpace(model.Password))
                error.Add(new { Input = "Password", Msg = "El campo password esta vacio" });
            if (string.IsNullOrWhiteSpace(model.ConfirmPassword))
                error.Add(new { Input = "Confirmar Password", Msg = "Debe confirmar el password" });
            if (model.Password != model.ConfirmPassword)
                error.Add(new { Input = "Password Fail", Msg = "Las contraseñas tienen que ser iguales" });
